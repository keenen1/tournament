import React from 'react'
import ResultsHourly from './ResultsHourly'

const ResultsDayOne = () => {
  return (
    <>
      <div className="category-pane">DAY ONE</div>
      <div className="results-day">
        <ResultsHourly
          hour={'9:00 AM'}

          court1cat={'U9'}
          court1teamA={'FLAT BUSH'}
          court1scoreA={24}
          court1teamB={'TGIF'}
          court1scoreB={8}

          court2cat={'U9'}
          court2teamA={'UFCSA WEST'}
          court2scoreA={9}
          court2teamB={'MAVS'}
          court2scoreB={29}

          court3cat={'OP'}
          court3teamA={'JBING'}
          court3scoreA={60}
          court3teamB={'TGIF'}
          court3scoreB={31}

          court4cat={'U15'}
          court4teamA={'MAVS'}
          court4scoreA={36}
          court4teamB={'UFCSA NORTH'}
          court4scoreB={17}

          court5cat={'35A'}
          court5teamA={'BARAKO EAST'}
          court5scoreA={36}
          court5teamB={'SOUTH AKL'}
          court5scoreB={47}

          court6cat={'35A'}
          court6teamA={'WEST PA MORE'}
          court6scoreA={49}
          court6teamB={'WEST SIDE'}
          court6scoreB={36}
        />
        <ResultsHourly
          hour={'9:50 AM'}

          court1cat={'OP'}
          court1teamA={'UNKNOWN'}
          court1scoreA={61}
          court1teamB={'FIL-KIWI'}
          court1scoreB={48}

          court2cat={'OP'}
          court2teamA={'MOUNT SISIG'}
          court2scoreA={37}
          court2teamB={'BRICK CITY'}
          court2scoreB={32}

          court3cat={'U11'}
          court3teamA={'FLAT BUSH'}
          court3scoreA={25}
          court3teamB={'MAVS'}
          court3scoreB={18}

          court4cat={'U11'}
          court4teamA={'TGIF'}
          court4scoreA={11}
          court4teamB={'UFCSA WEST'}
          court4scoreB={34}

          court5cat={'40A'}
          court5teamA={'WEST PA MORE'}
          court5scoreA={45}
          court5teamB={'TGIF MENTALITY'}
          court5scoreB={22}

          court6cat={'40A'}
          court6teamA={'KIWI ROOFING'}
          court6scoreA={37}
          court6teamB={'FIL-KIWI'}
          court6scoreB={30}
        />
        <ResultsHourly
          hour={'10:40 AM'}

          court1cat={'U13'}
          court1teamA={'TGIF'}
          court1scoreA={11}
          court1teamB={'UFCSA WEST A'}
          court1scoreB={92}

          court2cat={'U13'}
          court2teamA={'UFCSA WEST B'}
          court2scoreA={12}
          court2teamB={'MAVS'}
          court2scoreB={60}

          court3cat={'OP'}
          court3teamA={'AZTIGS BETA'}
          court3scoreA={84}
          court3teamB={'AUCKLANDER'}
          court3scoreB={52}

          court4cat={'OP'}
          court4teamA={'PHENOM'}
          court4scoreA={46}
          court4teamB={'TGIF'}
          court4scoreB={30}

          court5cat={'35A'}
          court5teamA={'AZTIGS 35'}
          court5scoreA={42}
          court5teamB={'BRICK CITY'}
          court5scoreB={50}

          court6cat={'40A'}
          court6teamA={'TGIF MAMBA'}
          court6scoreA={60}
          court6teamB={'JOKERS'}
          court6scoreB={36}
        />
        <ResultsHourly
          hour={'11:30 AM'}

          court1cat={'U15'}
          court1teamA={'UFCSA WEST'}
          court1scoreA={26}
          court1teamB={'MAVS'}
          court1scoreB={31}

          court2cat={'U17'}
          court2teamA={'UFCSA WEST'}
          court2scoreA={28}
          court2teamB={'MAVS'}
          court2scoreB={34}

          court3cat={'U19'}
          court3teamA={'UFCSA WEST'}
          court3scoreA={28}
          court3teamB={'MAVS'}
          court3scoreB={58}

          court4cat={'OP'}
          court4teamA={'PINOY PROPERTY'}
          court4scoreA={32}
          court4teamB={'BRICK CITY'}
          court4scoreB={18}

          court5cat={'40A'}
          court5teamA={'AZTIGS'}
          court5scoreA={57}
          court5teamB={'BARAKO EAST'}
          court5scoreB={36}

          court6cat={'OP'}
          court6teamA={'JBING'}
          court6scoreA={74}
          court6teamB={'SLAYERS'}
          court6scoreB={43}
        />
        <ResultsHourly
          hour={'12:50 PM'}

          court1cat={'40A'}
          court1teamA={'MAVS'}
          court1scoreA={0}
          court1teamB={'TGIF MENTALITY'}
          court1scoreB={0}

          court2cat={'U17'}
          court2teamA={'TGIF'}
          court2scoreA={0}
          court2teamB={'UFCSA WEST'}
          court2scoreB={0}

          court3cat={'U11'}
          court3teamA={'FLAT BUSH'}
          court3scoreA={0}
          court3teamB={'TGIF'}
          court3scoreB={0}

          court4cat={'U11'}
          court4teamA={'UFCSA WEST'}
          court4scoreA={0}
          court4teamB={'MAVS'}
          court4scoreB={0}

          court5cat={'35A'}
          court5teamA={'UFCSA WEST'}
          court5scoreA={0}
          court5teamB={'SOUTH AKL'}
          court5scoreB={0}

          court6cat={'35A'}
          court6teamA={'BARAKO EAST'}
          court6scoreA={0}
          court6teamB={'MAPUA CARDINALS'}
          court6scoreB={0}
        />
        <ResultsHourly
          hour={'1:40 PM'}

          court1cat={'U19'}
          court1teamA={'OUA'}
          court1scoreA={0}
          court1teamB={'AZTIGS DELTA'}
          court1scoreB={0}

          court2cat={'U15'}
          court2teamA={'TGIF'}
          court2scoreA={0}
          court2teamB={'FREE LANCER'}
          court2scoreB={0}

          court3cat={'40A'}
          court3teamA={'UFCSA SINULOG'}
          court3scoreA={0}
          court3teamB={'TGIF MAMBA'}
          court3scoreB={0}

          court4cat={'40A'}
          court4teamA={'UFCSA WEST'}
          court4scoreA={0}
          court4teamB={'BARAKO EAST'}
          court4scoreB={0}

          court5cat={'OP'}
          court5teamA={'PHENOM'}
          court5scoreA={0}
          court5teamB={'SLAYERS'}
          court5scoreB={0}

          court6cat={'35A'}
          court6teamA={'AZTIGS 35'}
          court6scoreA={0}
          court6teamB={'WEST SIDE'}
          court6scoreB={0}
        />
        <ResultsHourly
          hour={'2:30 PM'}

          court1cat={'U15'}
          court1teamA={'UFCSA WEST'}
          court1scoreA={0}
          court1teamB={'UFCSA NORTH'}
          court1scoreB={0}

          court2cat={'40A'}
          court2teamA={'FIL-KIWI'}
          court2scoreA={0}
          court2teamB={'SOUTH AKL'}
          court2scoreB={0}

          court3cat={'U9'}
          court3teamA={'FLAT BUSH'}
          court3scoreA={0}
          court3teamB={'UFCSA WEST'}
          court3scoreB={0}

          court4cat={'U9'}
          court4teamA={'TGIF'}
          court4scoreA={0}
          court4teamB={'MAVS'}
          court4scoreB={0}

          court5cat={'35A'}
          court5teamA={'BRICK CITY'}
          court5scoreA={0}
          court5teamB={'WEST PA MORE'}
          court5scoreB={0}

          court6cat={'U13'}
          court6teamA={'MAVS'}
          court6scoreA={0}
          court6teamB={'UFCSA WEST A'}
          court6scoreB={0}
        />
        <ResultsHourly
          hour={'3:20 PM'}

          court1cat={'U19'}
          court1teamA={'OUA'}
          court1scoreA={0}
          court1teamB={'MAVS'}
          court1scoreB={0}

          court2cat={'U17'}
          court2teamA={'TGIF'}
          court2scoreA={0}
          court2teamB={'UFCSA NORTH'}
          court2scoreB={0}

          court3cat={'OP'}
          court3teamA={'MOUNT SISIG'}
          court3scoreA={0}
          court3teamB={'PINOY PROPERTY'}
          court3scoreB={0}

          court4cat={'35A'}
          court4teamA={'UNKNOWN'}
          court4scoreA={0}
          court4teamB={'MAPUA CARDINALS'}
          court4scoreB={0}

          court5cat={'OP'}
          court5teamA={'WOW'}
          court5scoreA={0}
          court5teamB={'AUCKLANDER'}
          court5scoreB={0}

          court6cat={'U13'}
          court6teamA={'TGIF'}
          court6scoreA={0}
          court6teamB={'UFCSA WEST B'}
          court6scoreB={0}
        />
        <ResultsHourly
          hour={'4:10 PM'}

          court1cat={'OP'}
          court1teamA={'PHENOM'}
          court1scoreA={0}
          court1teamB={'JBING'}
          court1scoreB={0}

          court2cat={'OP'}
          court2teamA={'TGIF'}
          court2scoreA={0}
          court2teamB={'SLAYERS'}
          court2scoreB={0}

          court3cat={'40A'}
          court3teamA={'AZTIGS'}
          court3scoreA={0}
          court3teamB={'UFCSA WEST'}
          court3scoreB={0}

          court4cat={'40A'}
          court4teamA={'JOKER'}
          court4scoreA={0}
          court4teamB={'UFCSA SINULOG'}
          court4scoreB={0}

          court5cat={'40A'}
          court5teamA={'MAVS'}
          court5scoreA={0}
          court5teamB={'WEST PA MORE'}
          court5scoreB={0}

          court6cat={'40A'}
          court6teamA={'SOUTH AKL'}
          court6scoreA={0}
          court6teamB={'KIWI ROOFING'}
          court6scoreB={0}
        />
        <ResultsHourly
          hour={'5:00 PM'}

          court1cat={'U17'}
          court1teamA={'UFCSA NORTH'}
          court1scoreA={0}
          court1teamB={'MAVS'}
          court1scoreB={0}

          court2cat={'U15'}
          court2teamA={'TGIF'}
          court2scoreA={0}
          court2teamB={'MAVS'}
          court2scoreB={0}

          court3cat={'OP'}
          court3teamA={'WOW'}
          court3scoreA={0}
          court3teamB={'AZTIGS BETA'}
          court3scoreB={0}

          court4cat={'U19'}
          court4teamA={'AZTIGS DELTA'}
          court4scoreA={0}
          court4teamB={'UFCSA WEST'}
          court4scoreB={0}

          court5cat={'U15'}
          court5teamA={'UFCSA NORTH'}
          court5scoreA={0}
          court5teamB={'FREE LANCER'}
          court5scoreB={0}

          court6cat={'OP'}
          court6teamA={'UNKNOWN'}
          court6scoreA={0}
          court6teamB={'AKL HEAT'}
          court6scoreB={0}
        />
      </div>
    </>
  )
}

export default ResultsDayOne
